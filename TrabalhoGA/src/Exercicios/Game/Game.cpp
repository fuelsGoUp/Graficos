...
int main()
{
    ...
    const GLchar *rectFragmentShaderSource = R"glsl(
        #version 400
        out vec4 color;
    ...
    GLuint rectShaderID = glCreateProgram();
    glAttachShader(rectShaderID, rectVertexShader);
    glAttachShader(rectShaderID, rectFragmentShader);
    glLinkProgram(rectShaderID);
    ...
    glBindVertexArray(0);
		...
		for (const auto& plat : platforms) {
			...
			model = translate(model, vec3(plat.pos, 0.0f));
			model = scale(model, vec3(plat.size, 1.0f));
			glUniformMatrix4fv(glGetUniformLocation(rectShaderID, "model"), 1, GL_FALSE, value_ptr(model));
